      ...
      <section className="bg-paper py-28 md:py-40">
        <div className="mx-auto max-w-[1400px] px-6 md:px-10">
          ...
          <div className="mt-16 grid gap-6 md:grid-cols-12 md:gap-8">
            <AudienceCard index="01" tag="Athletes" title="A 10-year career demands a 60-year plan." body="We specialise in post-career life for the generation that peaks at 30." image={editorialAthlete} className="md:col-span-5 md:row-span-2" tall />
            <AudienceCard index="02" tag="Entertainers" title="From the studio to a legacy that outlasts the spotlight." body="Long-form planning for short-form fame." image={editorialEntertainer} className="md:col-span-7" />
            <AudienceCard index="03" tag="Tech Entrepreneurs" title="An exit is the beginning." body="Turn liquidity events into compounding generational wealth." image={editorialFounder} className="md:col-span-7" />
          </div>

          <div className="mt-14 flex justify-end">
            <Link to="/services" className="group inline-flex items-center gap-3 border-b border-ink pb-1 text-sm font-medium uppercase tracking-eyebrow">
        ...
        <div className="relative mx-auto max-w-[1400px] px-6 md:px-10">
          <div className="grid gap-16 md:grid-cols-12">
            <div className="md:col-span-5">
              ...
              <Link to="/services" className="mt-10 inline-flex items-center gap-3 text-sm font-medium uppercase tracking-eyebrow text-flame hover:text-paper transition-colors">
        ...
        <div className="mx-auto max-w-[1400px] px-6 md:px-10">
          <div className="grid gap-16 md:grid-cols-12 md:gap-12">
            <div className="md:col-span-7">
              ...
              <Link to="/about" className="mt-12 inline-flex items-center gap-3 border-b border-ink pb-1 text-sm font-medium uppercase tracking-eyebrow">
            ...
            <div className="md:col-span-5">
              <div className="aspect-[4/5] overflow-hidden bg-ink">
                <img src={editorialWatch} alt="Editorial detail" loading="lazy" className="h-full w-full object-cover" />
              </div>
      ...
      <section className="relative overflow-hidden bg-ink py-32 text-paper md:py-48">
        ...
        <div className="relative mx-auto grid max-w-[1400px] gap-16 px-6 md:grid-cols-12 md:px-10">
          <div className="md:col-span-7">
            ...
            <Link to="/" className="mt-12 inline-flex items-center gap-3 bg-paper px-7 py-4 text-sm font-medium uppercase tracking-eyebrow text-ink transition-transform hover:translate-y-[-2px]">
...
function AudienceCard({ index, tag, title, body, image, className = "", tall = false }: { index: string; tag: string; title: string; body: string; image: string; className?: string; tall?: boolean; }) {
  return (
    <article className={`group relative overflow-hidden bg-ink text-paper ${tall ? "min-h-[560px]" : "min-h-[340px]"} ${className}`}>
      <img src={image} alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover opacity-60 transition-all duration-700 group-hover:scale-105 group-hover:opacity-50" />
...
function CompoundingChart() {
  ...
  const r = 0.07;
  const monthly = 1;
  const seriesA: [number, number][] = [];
  const seriesB: [number, number][] = [];
  for (let y = 0; y <= years; y++) seriesA.push([y, monthly * Math.pow(1 + r, y)]);
  for (let y = 0; y <= years - 5; y++) seriesB.push([y + 5, monthly * Math.pow(1 + r, y)]);
  const max = Math.max(...seriesA.map((p) => p[1]));
  const x = (yr: number) => pad.l + (yr / years) * (w - pad.l - pad.r);
  const y = (v: number) => h - pad.b - (v / max) * (h - pad.t - pad.b);
  const path = (s: [number, number][]) => s.map((p, i) => `${i === 0 ? "M" : "L"}${x(p[0]).toFixed(1)},${y(p[1]).toFixed(1)}`).join(" ");
    ...
    <div className="rounded-sm border border-iron/20 bg-ink/40 p-4 md:p-6">
      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="h-auto w-full">
        ...
        {[0, 10, 20, 30, 40].map((yr) => (
          <text key={yr} x={x(yr)} y={h - 14} fill="rgba(205,197,186,0.6)" fontSize="10" textAnchor="middle">Year {yr}</text>
        ))}
